...
export default function NewCompetitionPage() {
  ...
  const onSubmit = async (data: CompetitionFormData) => {
    ...
    try {
      ...
      const { data: insertedCompetition, error: competitionError } = await supabase
        .from('competitions')
      ...
      if (competitionError) {
        ...
        alert(`대회 등록 중 오류가 발생했습니다: ${competitionError.message}`)
        return
      }

      // 2. 참가 그룹들 저장
      const groupsData = participationGroups.map(group => ({
        competition_id: insertedCompetition.id,
        name: group.name,
      ...
      if (groupsError) {
        console.error('Error creating participation groups:', groupsError)
        alert(`참가 그룹 등록 중 오류가 발생했습니다: ${groupsError.message}`)
        return
      }
      ...
      alert('대회 등록 중 오류가 발생했습니다. 다시 시도해주세요.')
    } finally {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between py-4">
            <div className="flex items-center">
              ...
              <h1 className="text-2xl font-bold text-gray-900">새 대회 등록</h1>
            </div>
